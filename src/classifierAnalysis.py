...
def runReplay(PcapDirectory, pacmodify, analyzerI):
    ...
    # Give 10s for the server to process the result and insert metrics into the database
    time.sleep(10)
...
def main(args):
    ...
    # Now try changing only the last GET pair
    ...
    Client = {0: ['Both sides are not differentiated when masked']}
    ...
    logger.info(' Client analysis' + str(Client) + '\n\t Server analysis' + str(Server) + '\r\n Number of Tests:' + str(
        Replaycounter))
    PRINT_ACTION(
        ' Client analysis' + str(Client) + '\n\t Server analysis' + str(Server) + '\r\n Number of Tests:' + str(
            Replaycounter), 0)
    ...
    if DPI == False:
        print((Client, Server))
        print(('\r\n No DPI based differentiation has been found within the first ', numPackets,
               ' packets being tested, exiting'))
        sys.exit()
