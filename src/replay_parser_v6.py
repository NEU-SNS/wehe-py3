...
import pickle, copy, re, random, string, hashlib
import ipaddress
import binascii
...
def getUDPstreamsMap(pcap_file, client_ip):
    command = 'tshark -r ' + pcap_file + ' -2 -R "udp" -T fields -e ipv6.src -e udp.srcport -e ipv6.dst -e udp.dstport > tmp'
    os.system(command)
    streams = set()
...
def createPacketMeta(pcapFile, outFile):
    command = ' '.join(['tshark -r', pcapFile,
                        ...
                        '-e ipv6.src', '-e tcp.srcport', '-e udp.srcport',
...
def extractStreams(pcap_file, follow_folder, client_ip, protocol, UDPstreamsMap=None):
    ...
    command = 'tshark -2 -R "not tcp.analysis.retransmission && not tcp.analysis.out_of_order" -r {} -w {}'.format(
        pcap_file, noRetransmitPcap)
    os.system(command)
    ...
    if protocol == 'tcp':
        ...
        for s in streams:
            csp = convert_ip(s.split(',')[0]) + '-' + convert_ip(s.split(',')[1])
            ...
            command = "tshark -r " + pcap_file + " -qz follow," + protocol + ",raw,0" + ' > ' + follow_folder + '/follow-stream-' + filename + '.txt'
            os.system(command)
...
def createHashLUT(clientQ, replay_name, numberOfHashed=5):
    ...
    for udp in clientQ:
        ...
        if seenCSP[udp.c_s_pair] < numberOfHashed:
            ...
            the_hash = hashlib.sha1(udp.payload.encode('ascii', 'ignore')).hexdigest()
...
def random_ascii_by_size(size):
    return ''.join(random.choice(string.ascii_letters + string.digits) for x in range(int(size))).encode()
...
def random_hex_by_size(size):
    ...
    assert (size % 2 == 0)
    size = size / 2
    payload_bytes = random_ascii_by_size(size)
    return binascii.hexlify(payload_bytes).decode('ascii', 'ignore')
...
def bitInv(hexPayload):
    bpayload = bin(int(hexPayload, 16))[2:].zfill(len(hexPayload) * 4)
    newb = ''
...
def bitInvNonHex(payload):
    ...
    hex_payload = binascii.hexlify(payload_bytes)
    bpayload = bin(int(hex_payload, 16))[2:].zfill(len(hex_payload) * 4)
    newb = ''
    for char in bpayload:
        if char == '0':
            newb += '1'
...
def random_hex_by_payload(hexPayload):
    ...
    if Configs().get('pureRandom'):
        ...
        if payload.startswith('GET'):
            ...
            req_hex = binascii.hexlify(req_bytes)
            return req_hex.decode('ascii', 'ignore')

        elif payload.startswith('HTTP'):
            res = Response(payload).createResponsePacket()
            res_bytes = res.encode()
            res_hex = binascii.hexlify(res_bytes)
            return res_hex.decode('ascii', 'ignore')
...
class Response(object):
    ...
    def createResponsePacket(self):
        return "{}\r\n{}\r\n\r\n".format(self.status, '\r\n'.join(
            [k[0] + ': ' + random_ascii_by_size(len(k[1])) for k in self.headers]))
    ...
    while end:
        ...
        while end:
            ...
            if p.talking == 's':
                ...
                clientQ.append(
                    RequestSet(tmpp.payload, tmpp.csp, ''.join([x.payload for x in resList]), tmpp.timestamp))
                break
...
def readNextPacket(streamMeta, streamHandle, randomPayload=False):
    ...
    while True:
        try:
            [talking, payload] = next(streamHandle)
            ...
            if p.length != len(payload) / 2:
                if '6279746573206d697373696e6720696e20636170747572652066696c655d' in payload:
                    ...
                    if DEBUG == 3: print(p.timestamp, '\t', p.talking, '=', talking, '\t', p.protocol, '\t', p.stream,
                                         '\t', p.length, '=', len(payload) / 2, '\t', payload)
                    sys.exit(-1)
...
class singlePacket(object):
    def __init__(self, desString, clientIP):
        ...
        if self.srcIP == clientIP:
            ...
            self.csp = convert_ip(self.srcIP + '.' + str(self.srcPort)) + '-' + convert_ip(
                self.dstIP + '.' + str(self.dstPort))
            ...
            self.csp = convert_ip(self.dstIP + '.' + str(self.dstPort)) + '-' + convert_ip(
                self.srcIP + '.' + str(self.srcPort))
...
def run(*args):
    ...
    with open(packetMeta, 'r') as f:
        for line in f:
            ...
            (talking, payload) = next(handles[dPacket.protocol][dPacket.stream])
            ...
            if talking == 'c':
                ...
                serverQ[dPacket.protocol][dPacket.csp].append(
                    UDPset(payload, dPacket.timestamp - serversTimeOrigin[dPacket.protocol][dPacket.csp], dPacket.csp))
    ...
    for stream in sorted(tcpMetas.keys()):
        ...
        decoded_client_payload = bytes.fromhex(TMPclientQ[0].payload).decode('ascii', 'ignore')
        if 'Host: static.ess.apple.com:80' in decoded_client_payload:
            streamSkippedList.append(stream)
        ...
        toHash = decoded_client_payload[:sample_size]
        # theHash = hash(toHash)
    ...
    print('\t#Server packets: {} (TCP: {}, UDP: {}) '.format(serverSideCount['tcp'] + serverSideCount['udp'],
                                                             serverSideCount['tcp'], serverSideCount['udp']))
